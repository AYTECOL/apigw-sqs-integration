...
```bash
api-gw-sqs$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
api-gw-sqs$ sam local start-api
api-gw-sqs$ curl http://localhost:3000/
```
...
```bash
api-gw-sqs$ sam logs -n HelloWorldFunction --stack-name api-gw-sqs --tail
```
...
```bash
sam delete --stack-name api-gw-sqs
```
